...
export async function listDisplayGames(tournamentId: string, courts?: string[], groupsOnly = false, includeHistory = false): Promise<Game[]> {
  if (dataMode === "local") {
  ...
  const historyIds = includeHistory || groupsOnly || !courts ? [] : games.filter((game) =>
