...
begin
 ...
 baseline_has_score=baseline_has_score or (tg_op='INSERT' and (public.protocol_has_score(current_snapshot) or public.protocol_history(target.point_history)<>'[]')),
 deleted=tg_op='DELETE' where game_id=target.id;
 return target;
end;
$$;

-- Reconcile summaries recorded before empty-history normalization was installed.
-- Audit events themselves remain untouched.
update public.game_protocols p set has_result_entry=true where not p.has_result_entry and exists(
 select 1 from public.game_protocol_events e where e.game_id=p.game_id and e.action='updated'
 and (e.history_change is null or e.history_change='{"drop":0,"keep":0,"append":[]}'::jsonb)
 and exists(select 1 from jsonb_each(e.changes) x where x.key like 'set%team_%' and coalesce(x.value->>'after','') not in ('','0'))
);
update public.game_protocols p set baseline_has_score=true where not p.baseline_has_score and exists(
 select 1 from public.game_protocol_events e where e.game_id=p.game_id and e.action in ('baseline','created')
 and e.history_change->'replace' is distinct from '[]'::jsonb
);
